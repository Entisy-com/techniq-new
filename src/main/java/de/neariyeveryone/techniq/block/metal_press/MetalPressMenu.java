...
import de.neariyeveryone.utilities.NResultSlotItemHandler;
...
public class MetalPressMenu extends AbstractContainerMenu {
    public MetalPressBlockEntity blockEntity;
    private Level level;
    private ContainerData data;

    public MetalPressMenu(int windowId, Inventory inv, ContainerLevelAccess access) {
        super(MenuTypes.METAL_PRESS_MENU.get(), windowId);
    }

    public MetalPressMenu(int windowId, Inventory inv, FriendlyByteBuf buffer){
        this(windowId,inv,inv.player.level.getBlockEntity(buffer.readBlockPos()), new SimpleContainerData(2));
    }
    public MetalPressMenu(int windowId, Inventory inv, BlockEntity entity, ContainerData data) {
        this(windowId, inv, ContainerLevelAccess.create(entity.getLevel(), entity.getBlockPos()));
        blockEntity = (MetalPressBlockEntity) entity;
        ...
        blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            ...
            addSlot(new SlotItemHandler(handler, 1, 61, 52));
            addSlot(new NResultSlotItemHandler(handler, 2, 97, 35));
        });
    ...
    public boolean stillValid(@NotNull Player player) {
        return stillValid(ContainerLevelAccess.create(level,
                blockEntity.getBlockPos()), player, Blocks.METAL_PRESS.get());
    }
