...
public class AdvancedFurnaceGeneratorMenu extends AbstractContainerMenu {
    public AdvancedFurnaceGeneratorBlockEntity blockEntity;
    private Level level;
    private ContainerData data;

    public AdvancedFurnaceGeneratorMenu(int windowId, Inventory inv, ContainerLevelAccess access) {
        super(MenuTypes.ADVANCED_FURNACE_GENERATOR_MENU.get(), windowId);
    }

    public AdvancedFurnaceGeneratorMenu(int windowId, Inventory inv, FriendlyByteBuf buffer){
        this(windowId, inv, inv.player.level.getBlockEntity(buffer.readBlockPos()), new SimpleContainerData(2));
    }
    public AdvancedFurnaceGeneratorMenu(int windowId, Inventory inv, BlockEntity entity, ContainerData data) {
        this(windowId, inv, ContainerLevelAccess.create(entity.getLevel(), entity.getBlockPos()));
        blockEntity = (AdvancedFurnaceGeneratorBlockEntity) entity;
    ...
    public boolean stillValid(@NotNull Player player) {
        return stillValid(ContainerLevelAccess.create(level,
                blockEntity.getBlockPos()), player, Blocks.ADVANCED_FURNACE_GENERATOR.get());
    }
    ...
    public AdvancedFurnaceGeneratorBlockEntity getBlockEntity() {
        return blockEntity;
    }
